...
// 여러 버튼을 동시에/연타로 눌러서 저장 요청이 겹치는 걸 막는 잠금
let isBusy = false;

// JSON 파일 수정: 저장 시점 sha가 이미 바뀌어있으면(예: 다른 탭에서 방금 수정함)
// 최신 상태를 다시 받아서 자동으로 한 번 더 시도합니다.
async function updateJsonFileWithRetry(path, transformFn, message, maxRetries = 2) {
  let lastError;
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      const fresh = await getJsonFile(path);
      const updatedJson = transformFn(fresh.json);
      await putFile(path, utf8ToBase64(JSON.stringify(updatedJson, null, 2)), fresh.sha, message);
      return updatedJson;
    } catch (e) {
      lastError = e;
      if (!/does not match|sha/i.test(e.message)) throw e;
      // sha 충돌이면 잠깐 대기 후 최신 상태로 다시 시도
...
document.getElementById("member-form").addEventListener("submit", async (e) => {
  ...
  try {
    const updated = await updateJsonFileWithRetry(
      "data/members.json",
      (json) => json.map((m) => (m.id === id ? { ...m, name, role, intro, likes } : m)),
      `구성원 프로필 수정: ${name}`
    );
  ...
  try {
    ...
    if (editingPostId) {
      ...
      const updated = await updateJsonFileWithRetry(
        "data/posts.json",
        (json) => {
          const original = json.find((p) => p.id === editingPostId);
          removedImages = (original?.images || []).filter((img) => !finalImages.includes(img));
          return json.map((p) =>
            p.id === editingPostId
