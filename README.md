...
## 6. 홈페이지 안에서 사진/글 올리고 지우기 (관리자 페이지)

`/admin.html`에 접속하면 GitHub 웹 화면을 거치지 않고 바로 사진을 올리거나
일기를 쓰고, 기존 기록을 지울 수 있어요. 내부적으로는 여전히 GitHub에
커밋하는 방식이라 GitHub 개인 액세스 토큰이 한 번 필요합니다 (최초 1회만,
이후에는 이 브라우저에 저장돼서 다시 안 물어봐요).

1. `admin.html` 접속 → 안내에 따라 [GitHub 토큰 발급 페이지](https://github.com/settings/personal-access-tokens/new)에서
   `raphale` 저장소에 대해 **Contents: Read and write** 권한을 가진 토큰 발급
2. 발급받은 토큰을 붙여넣고 저장
3. 이후엔 폼에 채워서 저장하면 자동으로 커밋 → 1~2분 뒤 사이트에 반영

주의: 이 토큰을 가진 사람은 누구나 사이트 내용을 바꿀 수 있어요. 가족 외
다른 사람과 공유하는 기기에서는 로그아웃(토큰 삭제) 버튼을 눌러두세요.
